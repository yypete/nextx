        ...
        <div class="h-full bg-white rounded-lg border bordered-gray-400 w-full">
          ...
          <div class="flex items-center justify-between px-10 py-10">
            ...
            <div class="flex items-center space-x-2">
              <button class="btn-tool relative" data-tooltip="读取文件"><i class="fi fi-rr-book-circle-arrow-right pr-1"></i>读取文件 </button>
              <button class="btn-tool" data-tooltip="粘贴"><i class="fi fi-tr-paste pr-1"></i>粘贴</button>
              <button class="btn-tool" data-tooltip="清空" @click="clearOut"><i class="fi fi-rr-broom pr-1"></i>清空</button>
              <button class="btn-tool" data-tooltip="复制"><i class="fi fi-sr-copy-alt pr-1"></i>复制</button>
            </div>
          </div>
          <!-- tool-content -->
          <div>
            <div class="w-full flex justify-center">
              <input type="text" class="rounded-lg border-gray-500 border h-28 w-4/5 mx-10 text-2xl pb-14 pl-4" v-model="inputText" @input="calculateRipeMd160Hash">
            </div>
            <div class="flex items-center justify-between px-10 py-10">
            <!-- left-title -->
              ...
              <!-- 分割符 -->
                <div class="divide flex items-center space-x-2">
                  <div class="flex items-center relative">
                    分隔符：
                    <button class="border border-gray-500 px-2 py-1 flex items-center" @click="toggleDropdown">
                      <div>(无分隔符)</div>
                      <i class="fi fi-rs-angle-small-down ml-2"></i>
                      <div v-if="isDropdownOpen" class="absolute top-10 mt-1 bg-white border border-gray-500 rounded shadow-md dropdown-menu w-26 z-10">
                        <div @click="selectOption('冒号')" class="dropdown-item px-4 py-2 hover:bg-gray-200 cursor-pointer">冒号(:)</div>
                        <div @click="selectOption('空格')" class="dropdown-item px-4 py-2 hover:bg-gray-200 cursor-pointer">空格()</div>
                        <div @click="selectOption('中划线')" class="dropdown-item px-4 py-2 hover:bg-gray-200 cursor-pointer">中划线(-)</div>
                      </div>
                    </button>
                  </div>
                  <div>
                    <button class="border-gray-500 border rounded-lg px-2 py-1 rounded-r-none" @click="selectOption('小写')">小写</button>
                    <button class="border-gray-500 border rounded-lg px-2 py-1 rounded-l-none" @click="selectOption('大写')">大写</button>
                  </div>
                  <div>
                    <button class="border-gray-500 border rounded-lg px-2 py-1 rounded-r-none" @click="selectOption('HEX')">HEX</button>
                    <button class="border-gray-500 border rounded-lg px-2 py-1 rounded-l-none" @click="selectOption('BASE64')">BASE64</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="w-full flex justify-center relative">
              <input type="text" class="rounded-lg border-gray-500 border h-14 w-4/5 mx-10 text-sm" v-model="outputText">
              <a href="#" class=""><i class="fi fi-rr-copy-alt absolute right-32 top-4 text-2xl"></i></a>
            </div>
        </div>
        </div>
        <!-- page-nav -->
  ...
  methods: {
    ...
    calculateRipeMd160Hash() {
      if (this.inputText.trim() === "") {
        ...
        this.outputText = this.ripeMd160(this.inputText).toString().substring(0, 40);
      }
    },
    toggleDropdown() {
